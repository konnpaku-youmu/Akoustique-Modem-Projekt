...
psd_spectrogram = 10 * log(rescale(psd_spectrogram, 0, 1));
...
% calculate the PSD by definition
subplot(2, 2, 4);

p_out = out.^2; % w(t) = v(t)^2
p_fft = stft(p_out, fs, "FFTLength", N, "FrequencyRange","onesided", 'OverlapLength', 64);
psd = abs(p_fft) ./ N;

psd_def = imagesc(psd);
set(gca,'YDir','normal');
xticklabels = 0:0.5:nbsecs;
xticks = linspace(1, size(psd, 2), numel(xticklabels));
set(gca, 'XTick', xticks, 'XTickLabel', xticklabels);
yticklabels = 0:2:(fs/2)/1000;
yticks = linspace(1, size(psd, 1), numel(yticklabels));
set(gca, 'YTick', yticks, 'YTickLabel', yticklabels)
xlabel('Time (s)'); ylabel('Frequency (kHz)');
colorbar;
